//import libraries
import React, { Component } from 'react';
import { 
  View, 
  Text, 
  StyleSheet, 
  TouchableOpacity, 
  Image,
  ImageBackground
 } from 'react-native';
import {
  Container,
  Content,
  Icon,
  Card,
  CardItem,
  Button
} from 'native-base'
import { Actions, ActionConst } from 'react-native-router-flux';
import { connect } from 'react-redux';

import I18n from '../../langs';
import Statusbar from '../../component/Statusbar'
import MyHeader from '../../component/MyHeader'
import Share, {ShareSheet} from 'react-native-share';
// create a component
class Invite extends Component {

  constructor(props) {
    super(props);
    this.state = {
      visible: false
    }
  }

  onCancel() {
    console.log("CANCEL")
    this.setState({visible:false});
  }

  onOpen() {
    console.log("OPEN")
    this.setState({visible:true});
  }

  render() {
    let shareOptions = {
      title: "有個好Apps介紹比你",
      message: '朋友仔，有個好Apps介紹比你，快啲去下載啦。 Android:http://www.google.com/efunsim.com/  iOS:https://www.apple.com/hk/itunes/efunsim.com/',
      subject: '朋友仔，有個好Apps介紹比你，快啲去下載啦。 Android:http://www.google.com/efunsim.com/  iOS:https://www.apple.com/hk/itunes/efunsim.com/' //  for email
    };

    let shareImageBase64 = {
      title: "React Native",
      message: "Hola mundo",
      url: REACT_ICON,
      subject: "Share Link" //  for email
    };

    return (
      <Container style={styles.container}>
        <Statusbar backgroundColor={global.colors.background.statusBar}/>
        <View style={{ backgroundColor:global.colors.background.statusBar}}>
          <MyHeader 
            title={I18n.t('Invite')}
            titleColor={global.colors.text.t1}
          />
        </View>
        <Content>
          {/* 
          <View  style={{alignItems:'center', justifyContent:'center' }}>
            <Text style={{fontSize:30, fontWeight:'bold', color:'#2d2343', margin:10}}>{I18n.t('Referral a Friend')}</Text>
            <Text style={{fontSize:24, fontWeight:'600', color:'#2d2343', margin:10}}>{I18n.t('Invite friends, earn free points, get more free products')}</Text>
            <Text style={{fontSize:20, fontWeight:'400', color:'#2d2343', margin:10}}>{I18n.t('Invite your friends description')}</Text>
          </View> 
          */}
          <View style={{flex:1,backgroundColor:'#FB96C4'}}>
            <View style={{flexDirection:'row', flex:1, marginTop:25 }} >
              <ImageBackground 
                resizeMode="contain"
                source={require('../../images/invite.png')}
                style={{width:"100%", height:190 * global.scale, resizeMode:'contain', alignSelf: 'stretch',}}
              >
                <View style={{width:"100%", flexDirection:'row', height:88 * global.scale, marginTop:95 * global.scale, marginLeft:5, marginRight:5}}>
                  <View style={{width:120 * global.scale, marginRight:20 * global.scale, alignItems:'center', justifyContent:'center'}}>
                    <Text style={{fontSize:22* global.scale, textAlign:'center', color:'#434371', fontWeight:'bold', flexWrap:'wrap'}}>{I18n.t('Invite Friends')}</Text>
                  </View>
                  <View style={{width:120 * global.scale, marginRight:33 * global.scale, alignItems:'center', justifyContent:'center'}}>
                    <Text style={{fontSize:22* global.scale, textAlign:'center', color:'#434371', fontWeight:'bold', flexWrap:'wrap'}}>{I18n.t('Friends Install APP')}</Text>
                  </View>
                  <View style={{width:120 * global.scale, alignItems:'center', justifyContent:'center', marginRight:5}}>
                    <Text style={{fontSize:22* global.scale, textAlign:'center', color:'#434371', fontWeight:'bold', flexWrap:'wrap'}}>{I18n.t('Get Rewarded')}</Text>
                  </View>
                </View>
              </ImageBackground>
            </View>
            <View style={{marginTop:10,alignItems:'center',justifyContent:'center', marginBottom:70 }}>
              <Text style={{marginTop:10, fontSize:20* global.scale, color:'#434371', fontWeight:'bold'}}>{I18n.t('Friends get')}10<Icon type='Octicons' name="ruby" style={{fontSize: 25* global.scale, color: global.styles.ruby, marginLeft:0, alignSelf:'flex-end', marginBottom:5}}/> {I18n.t('when_they_install_app')}</Text>
              <Text style={{marginTop:10, fontSize:20* global.scale, color:'#434371', fontWeight:'bold'}}>{I18n.t('You get')}50<Icon type='Octicons' name="ruby" style={{fontSize: 25* global.scale, color: global.styles.ruby, marginLeft:0, alignSelf:'flex-end', marginBottom:5}}/> {I18n.t('when_they_install_app')}</Text>
              <Text style={{marginTop:10, fontSize:20* global.scale, color:'#434371', fontWeight:'bold'}}>{I18n.t('You get')}100<Icon type='Octicons' name="ruby" style={{fontSize: 25* global.scale, color: global.styles.ruby, marginLeft:0, alignSelf:'flex-end', marginBottom:5}}/> {I18n.t('when_they_shop')}</Text>
            </View>
          </View>
          <View style={{width:'100%',alignItems:'center',justifyContent:'center'}}>
            <View style={{top:-40}}>
              <View style={{backgroundColor:'#fff', alignItems:'center', justifyContent:'center', padding: 10, borderColor:'#ccc', borderRadius:10, borderWidth:1}}>
                <Text style={{color:'#434371', fontSize:20* global.scale}}>{I18n.t('Your Referral Code')}</Text>
                <Text style={{color:'#434371', fontSize:20* global.scale, marginTop:5 }}>{this.props.user.user.custom_attributes.user_code}</Text>
              </View>
            </View>
          </View>
          <View style={{flex:1,flexDirection:'row', flexWrap:'wrap', margin: 20* global.scale, marginTop: 0* global.scale, paddingBottom:20, alignSelf:'center' }}> 
            <View style={{alignItems:'center',justifyContent:'center'}}>
              <Button 
                transparent
                style={styles.btn}
                onPress={()=>{
                  this.onCancel();
                  setTimeout(() => {
                    Share.shareSingle(Object.assign(shareOptions, {
                      "social": "twitter"
                    }))
                    .then((res) => { console.log(res) })
                    .catch((err) => { err && console.log(err); });
                  },300);
                }}>
                  <Image source={require('../../images/twitterhover_pc.png')} style={{width:50,height:50}} />
                  <Text style={{fontWeight:'bold'}}>Twitter</Text>
                </Button>
            </View>
            <View style={{alignItems:'center',justifyContent:'center'}}>
                <Button 
                  transparent
                  style={styles.btn}
                  onPress={()=>{
                    this.onCancel();
                    setTimeout(() => {
                      Share.shareSingle(Object.assign(shareOptions, {
                        "social": "facebook"
                      }))
                      .then((res) => { console.log(res) })
                      .catch((err) => { err && console.log(err); });
                    },300);
                }}>
                    <Image source={require('../../images/facebook.png')} style={{width:50,height:50}} />
                    <Text style={{fontWeight:'bold'}}>Facebook</Text>
                </Button>
            </View>
            <View style={{alignItems:'center',justifyContent:'center'}}>
                <Button 
                  transparent
                  style={styles.btn}
                  onPress={()=>{
                    this.onCancel();
                    setTimeout(() => {
                      Share.shareSingle(Object.assign(shareOptions, {
                        "social": "whatsapp"
                      }))
                  .then((res) => { console.log(res) })
                  .catch((err) => { err && console.log(err); });
                    },300);
                  }}>
                    <Image source={require('../../images/whatsapp.png')} style={{width:50,height:50}} />
                    <Text style={{fontWeight:'bold'}}>Whatsapp</Text>
                </Button>
            </View>
            {/* <View style={{alignItems:'center',justifyContent:'center'}}>
                <Button
                  transparent
                  style={styles.btn}
                  onPress={()=>{
                      this.onCancel();
                      setTimeout(() => {
                        Share.shareSingle(Object.assign(shareOptions, {
                          "social": "email"
                        }))
                        .then((res) => { console.log(res) })
                        .catch((err) => { err && console.log(err); });
                      },300);
                    }}
                  >
                  <Image source={require('../../images/email.png')} style={{width:50,height:50}} />
                  <Text>Email</Text>
                </Button>
                
            </View> */}
            <View style={{alignItems:'center',justifyContent:'center'}}>
              <Button
                transparent
                style={styles.btn}
                onPress={()=>{
                  this.onCancel();
                  setTimeout(() => {
                    Share.open(shareOptions)
                    .then((res) => { console.log(res) })
                    .catch((err) => { err && console.log(err); });
                  },300);
                }}>
                  <Image source={require('../../images/more1.png')} style={{width:50,height:50}} />
                  <Text style={{fontWeight:'bold'}}>More</Text>
                </Button>
                </View>
          </View>
        </Content>
      </Container>
      // <View/>
    );
  }
}

// define your styles
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: global.styles.body,
  },
  btn:{
    flexDirection:"column",
    marginLeft:14,
  }
});

//  twitter icon
const TWITTER_ICON = "data:image/png;base64,";

//  facebook icon
const FACEBOOK_ICON = "data:image/png;base64,";

//  whatsapp icon
const WHATSAPP_ICON = "data:image/png;base64,";

//  gplus icon
const GOOGLE_PLUS_ICON = "data:image/png;base64,";

//  email icon
const EMAIL_ICON = "data:image/png;base64,";

//  pinterest icon
const PINTEREST_ICON = "data:image/png;base64,"

//  clipboard icon
const CLIPBOARD_ICON = "data:image/png;base64,";

//  more icon
const MORE_ICON = "data:image/png;base64,";


const REACT_ICON = 'data:image/png;base64,';

//make this component available to the app
const mapStateToProps = ({ user }) => ({ user })
export default connect( mapStateToProps, {})(Invite);
